...
export class AppEffects {
  ...
  loadNews$ = createEffect((actions) =>
    actions.pipe(
      ...
      tap((news) =>
        this._appRepository.updateNews({ data: news, loading: false })
      )
    )
  );
  ...
  loadNewsDetails$ = createEffect((actions) =>
    actions.pipe(
      ...
      tap((newsDetails) =>
        this._appRepository.updateNewsDetails({
          data: newsDetails,
          loading: false,
        })
      )
  ...
  loadActivePlayers$ = createEffect((actions) =>
    actions.pipe(
      ...
      tap((activePlayers) =>
        this._appRepository.updateActivePlayers({
          data: activePlayers,
          loading: false,
        })
      )
  ...
  loadUserTeam$ = createEffect((actions) =>
    actions.pipe(
      ...
      tap(([action, userTeam]) => {
        return this._appRepository.updateUserTeam({
          data: !userTeam ? userTeam : userTeam?.data,
          loading: true,
        });
      }),
      switchMap(([action, userTeam]) => {
        if (userTeam !== null) {
          return of(userTeam).pipe(
            tap(() =>
              this._appRepository.updateUserTeam({
                data: userTeam.data,
                loading: false,
              })
            )
  ...
  createUserTeam$ = createEffect((actions) =>
    actions.pipe(
      ...
      switchMap((action) =>
        this._apiService.createUserTeam(action.name).pipe(
          map((response) => ({ userTeam: response, error: null })),
          catchError((error) => {
            return of({
              userTeam: null,
              error: error.error.error.statusMessage,
            });
          })
        )
      ),
  ...
  updateUserTeam$ = createEffect((actions) =>
    actions.pipe(
      ...
      withLatestFrom(this._appRepository.selectUserTeam()),
      tap(([action, userTeam]) => {
        // reorganize actions in order to avoid unnecessary selects from store
        return this._appRepository.updateUserTeam({
          data: !userTeam ? userTeam : userTeam?.data,
          loading: true,
        });
